...
int interval = 3;     // Interval between writes in ms, frequency = 1000/interval (approx.)
...
/* ---------------------------------- Setup ----------------------------------------*/
void setup() {
  //  pinMode(InterruptTrimPin,OUTPUT);
  pinMode(IMU1,OUTPUT);
  pinMode(IMU2,OUTPUT);
  pinMode(IMU3,OUTPUT);
...
void loop() {
  ...
  if (currentTime - prevTime >= interval && imu_flag == 1)
  {
    ...
    //IMU select
    digitalWrite(imu_sel%3+3,LOW);
    digitalWrite((imu_sel+1)%3+3,HIGH);
    digitalWrite((imu_sel+2)%3+3,HIGH);
    ...
    switch (imu_id) {
      ...
      case '2':
        ...
        sprintf(accBuf, "I%d%c%05d%c%05d%c%05d%c%05dU",(imu_sel%3), sgn(ax), abs(ax), sgn(ay), abs(ay), sgn(az), abs(az), sgn(gz), abs(gz));
        break;
      case '1':
